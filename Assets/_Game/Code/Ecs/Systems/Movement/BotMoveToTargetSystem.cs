...
namespace Game.Ecs.Systems.Movement {
    ...
    internal partial struct BotMoveToTargetSystem : ISystem {
        private EntityQuery playersQuery;

        [BurstCompile]
        public void OnCreate(ref SystemState state) {
            state.RequireForUpdate(SystemAPI.QueryBuilder()
                .WithAll<BotControlTag, BotTarget>().Build());
        }
        ...
        public void OnUpdate(ref SystemState state) {
            ...
            foreach (var (transform, speed, target, targetEnabled) in
                     SystemAPI.Query<RefRW<LocalTransform>, RefRO<MoveSpeed>, RefRW<BotTarget>, EnabledRefRW<BotTarget>>()
